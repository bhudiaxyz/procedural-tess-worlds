...
import GOLDEN_RATIO from "../utils/MathUtils";


export default class IcosahedronGeometryGenerator extends AbstractGenerator {
  ...
  generate(props) {
    ...
    function createIcosahedron() {
      ...
      vertices.push(createVertex(0, 1, GOLDEN_RATIO));
      vertices.push(createVertex(0, 1, -GOLDEN_RATIO));
      vertices.push(createVertex(0, -1, GOLDEN_RATIO));
      vertices.push(createVertex(0, -1, -GOLDEN_RATIO));

      vertices.push(createVertex(1, GOLDEN_RATIO, 0));
      vertices.push(createVertex(1, -GOLDEN_RATIO, 0));
      vertices.push(createVertex(-1, GOLDEN_RATIO, 0));
      vertices.push(createVertex(-1, -GOLDEN_RATIO, 0));

      vertices.push(createVertex(GOLDEN_RATIO, 0, 1));
      vertices.push(createVertex(-GOLDEN_RATIO, 0, 1));
      vertices.push(createVertex(GOLDEN_RATIO, 0, -1));
      vertices.push(createVertex(-GOLDEN_RATIO, 0, -1));
    ...
    function tessellate() {
      for (let i = 2; i <= subdivisions; i++) {
        ...
        for (let j = 0; j < faces.length; j++) {
          let m1 = getMidpoint(faces[j]['a'], faces[j]['b']);
          let m2 = getMidpoint(faces[j]['b'], faces[j]['c']);
          let m3 = getMidpoint(faces[j]['c'], faces[j]['a']);

          let m1Idx = vertices.indexOf(m1);
          if (m1Idx === -1) {
            vertices.push(m1);
            m1Idx = vertices.length - 1;
          }

          let m2Idx = vertices.indexOf(m2);
          if (m2Idx === -1) {
            vertices.push(m2);
            m2Idx = vertices.length - 1;
          }

          let m3Idx = vertices.indexOf(m3);
          if (m3Idx === -1) {
            vertices.push(m3);
    ...
    // Add centroids of triangles to the dictionary's value list that use the dictionary's key vertex
